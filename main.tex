...
\usepackage{parskip}
\usepackage{enumitem}
% \usepackage{hyperref}  % BROKE EVERYTHING -- commenting out until i figure it out
% the error was: "Option clash for package hyperref" -- no idea why
% tried moving it around, still broken. leaving it out for now
...
but false statements. Maynez et al.\ (2020) document this rigorously. OpenAI
(2023) shows that prompting strategy substantially influences accuracy. Two
mitigation approaches: structured prompting (Li et al., 2023) and
retrieval-augmented generation (Lewis et al., 2020). SimplrAI combines both.

\subsection{Reading-level adaptation and interface design}

The Lexile Framework (MetaMetrics, 2019) shows comprehension falls sharply when
text exceeds a learner's threshold. Xu et al.\ (2023) show LLMs default to
college-level prose unless constrained. Nielsen Norman Group (2023) and Reich
(2020) find students learn best from interfaces with low visual noise and chunked
content. Quinn et al.\ (2023) argue that dense paragraphs are suboptimal for
K--12 learners.
...
learning are understudied. SimplrAI is engineered to address all three.
